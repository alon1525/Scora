...
interface MatchEvent {
  ...
  status: 'upcoming' | 'betting_open' | 'betting_closed' | 'completed';
  home_score?: number;
  away_score?: number;
}
...
export const MatchPredictions = () => {
  ...
  const [completedMatches, setCompletedMatches] = useState<MatchEvent[]>([]);
  const [predictions, setPredictions] = useState<MatchPrediction[]>([]);
  const [completedPredictions, setCompletedPredictions] = useState<Record<string, MatchPrediction[]>>({});
  ...
  const fetchCurrentMatch = async () => {
    try {
      ...
      if (data && data.length > 0) {
        setCurrentMatch(data[0] as MatchEvent);
        return;
      }
      ...
      if (closedData && closedData.length > 0) {
        setCurrentMatch(closedData[0] as MatchEvent);
        return;
      }
      ...
      if (shouldCloseData && shouldCloseData.length > 0) {
        ...
        setCurrentMatch({ ...shouldCloseData[0], status: 'betting_closed' as const });
        return;
      }
      ...
      if (upcomingData && upcomingData.length > 0) {
        ...
        if (updateError) throw updateError;
        setCurrentMatch({ ...upcomingData[0], status: 'betting_open' as const });
      }
    } catch (error) {
  ...
  const fetchCompletedMatches = async () => {
    try {
      ...
      if (error) throw error;
      setCompletedMatches((data || []) as MatchEvent[]);
    } catch (error) {
  ...
  const fetchMatchPredictions = async (matchId: string) => {
    try {
      const { data, error } = await supabase
        .from('match_predictions')
        .select('*')
  ...
  const calculateTableScores = async () => {
    try {
      ...
      const real = realData.positions as Record<string, number>;
  ...
  const deletePrediction = async (predictionId: string, predictionUsername: string) => {
  ...
  const setWinner = async (winnerUsername: string) => {
    if (!currentMatch) return;
