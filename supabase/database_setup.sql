...
BEGIN
  ...
  SELECT COALESCE(SUM(points_earned), 0) INTO fixture_points
  FROM (
    SELECT (value->>'points_earned')::INTEGER as points_earned
    FROM jsonb_each(user_profile.fixture_predictions)
  ) as points;
  ...
  SET 
    fixture_points = user_profile.fixture_points,
    table_points = user_profile.table_points,
