-- Multi-Season Support Database Schema
-- This script adds multi-season support to the existing database
...
-- Add season to user_profiles (for current season stats)
ALTER TABLE user_profiles 
ADD COLUMN IF NOT EXISTS current_season VARCHAR(7) DEFAULT '2024-25';

-- Add season to predictions tables
ALTER TABLE user_table_predictions 
ADD COLUMN IF NOT EXISTS season VARCHAR(7) DEFAULT '2024-25';

ALTER TABLE user_match_predictions 
ADD COLUMN IF NOT EXISTS season VARCHAR(7) DEFAULT '2024-25';

-- Add season to fixture_comments
...
-- Function to check if season transition is needed
...
-- Function to update user best stats
...
-- Function to reset user data for new season
CREATE OR REPLACE FUNCTION reset_user_data_for_new_season(p_new_season VARCHAR(7))
RETURNS VOID AS $$
BEGIN
    -- Update current season in user profiles
    ...
    -- Clear old season predictions
    DELETE FROM user_table_predictions WHERE season != p_new_season;
    DELETE FROM user_match_predictions WHERE season != p_new_season;
...
CREATE INDEX IF NOT EXISTS idx_user_table_predictions_season ON user_table_predictions(season);
CREATE INDEX IF NOT EXISTS idx_user_match_predictions_season ON user_match_predictions(season);
